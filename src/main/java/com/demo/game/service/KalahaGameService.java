...
public class KalahaGameService {
    ...
    public PlayStatus play(Integer gameId, Integer pitId) {
        ...
        PlayerStatus playerPlayerStatus = kalahaGame.getPlayerStatus();
        if (!PlayerStatus.isPlayerOne(playerPlayerStatus) && !PlayerStatus.isPlayerTwo(playerPlayerStatus)) {
            throw new InvalidPlayerTurn("Game Over and Winner player is  " + playerPlayerStatus);
        }
        int pit = pitId.intValue() - 1;
        if (!isValidMove(kalahaGame, pit))
            throw new InvalidPlayerTurn("For Player " + playerPlayerStatus + " not a valid pit " + pitId);
        if (PlayerStatus.isPlayerOne(playerPlayerStatus)) {
            play(kalahaGame, pit, kalahaGame.getPlayer1());
        } else if (PlayerStatus.isPlayerTwo(playerPlayerStatus)) {
    ...
    private PlayStatus createPlayStatus(KalahaGame kalahaGame) {
        ...
        return new PlayStatus().id(kalahaGame.getId()).uri(String.format(Constant.uriGame, kalahaGame.getId())).status(statusMap);
    }
    ...
    private void play(KalahaGame kalahaGame, int pit, KalahPlayer player) {
        ...
        if (!player.ownKalaha(lastDropPit))
            updateNextTurn(kalahaGame);
